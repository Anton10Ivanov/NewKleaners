 ...
 * to semantic color tokens using Node.js built-in modules.
 */
...
async function migrateComponents(): Promise<MigrationStats> {
  ...
  try {
    // Find all TypeScript files in components and app directories
    const componentFiles = findTsFiles('components');
    const appFiles = findTsFiles('app');
